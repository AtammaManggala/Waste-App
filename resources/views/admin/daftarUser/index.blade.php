            ...
            <table class="table table-hover mb-0 table-rounded table-line">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $user->name }}</td>
                        <td>{{ $user->email }}</td>
                        <td>{{ $user->jml_botol ?? 0 }}</td>
                        <td>{{ $user->countTransaction->jml_point ?? 0 }}</td>
                        <td>{{ ($user->jml_botol ?? 0) * $masterPoint }}</td>
                        <td>{{ $user->role }}</td>
                        <td>
                            <a href="{{route('daftarUser.edit',$user->id)}}"
                            class="btn btn-warning">update</a> 
                            | 
                            <a href="{{route('daftarUser.delete',$user->id)}}" 
